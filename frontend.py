...
# Automatically refresh the app every 10 minutes
st_autorefresh(interval=600000, key="auto_refresh")  ### NEW
...
def fetch_ctd_data():
    docs = db.collection('CTD').order_by('timestamp').get()
...
# Plot
if not df.empty:
    fig = px.line(df, x='datetime', y='temperature', title='Live CTD Data')
    fig.update_traces(connectgaps=False)
    fig.update_layout(xaxis_title='Time', yaxis_title='Temperature (°C)')
    st.plotly_chart(fig, use_container_width=True)
else:
    st.warning("No CTD data available.")
