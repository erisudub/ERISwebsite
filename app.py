...
if file_path:
    try:
        # Load data
        ctd_data = pd.read_csv(file_path, delimiter='\t')  # Adjust delimiter if needed

        # Convert 'date' to datetime if it exists
        if 'date' in ctd_data.columns:
            ctd_data['date'] = pd.to_datetime(ctd_data['date'], errors='coerce')

        # Convert relevant columns to numeric safely
        numeric_cols = ['temperature', 'conductivity', 'par', 'turbidity', 'salinity', 'pressure', 'oxygen', 'depth']
        for col in numeric_cols:
            if col in ctd_data.columns:
                ctd_data[col] = pd.to_numeric(ctd_data[col], errors='coerce')

        # Dropdown for variables to plot
        available_vars = [col for col in numeric_cols if col in ctd_data.columns and col != 'depth']
        selected_vars = st.multiselect("Select variables to plot against depth:", available_vars, default=available_vars[:2])

        # Depth filtering if depth is present
        if 'depth' in ctd_data.columns:
            min_depth, max_depth = float(ctd_data['depth'].min()), float(ctd_data['depth'].max())
            depth_range = st.slider("Select depth range", min_value=min_depth, max_value=max_depth, value=(min_depth, max_depth))
            filtered_data = ctd_data[(ctd_data['depth'] >= depth_range[0]) & (ctd_data['depth'] <= depth_range[1])]
        else:
            filtered_data = ctd_data
        ...
        fig.update_layout(
            title="CTD Profile: Selected Variables vs Depth",
            xaxis_title="Depth (m)",
            yaxis_title="Value",
            yaxis=dict(range=[0, None]),  # Set y-axis to start at 0
