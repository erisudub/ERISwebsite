...
weather_csv_file_path = '/content/weatherdata.csv'  # Replace with the actual path of the Weather CSV
...
# Plot Your Own Data Page
   ...
   # File uploader for user-provided CSV files
   ...
   if user_file:
       try:
           ...
           if 'time' in user_data.columns:
               ...
               start_date_user = st.date_input("Start Date", value=user_data['time'].min().date())
               end_date_user = st.date_input("End Date", value=user_data['time'].max().date())


               # Filter the data based on the selected date range
               filtered_user_data = user_data[(user_data['time'] >= pd.Timestamp(start_date_user)) &
                                              (user_data['time'] <= pd.Timestamp(end_date_user))]


               # Display the filtered data
               st.write(f"### Data from {start_date_user} to {end_date_user}")
               st.dataframe(filtered_user_data)


               # Plot the user's data
               fig_user = go.Figure()
               for col in filtered_user_data.columns:
                   if col != 'time':  # Don't plot the time column
                       fig_user.add_trace(go.Scatter(x=filtered_user_data['time'],
                                                    y=filtered_user_data[col],
                                                    mode='lines', name=col))


               fig_user.update_layout(
                   title="User Data Over Time",
                   xaxis_title="Time",
                   yaxis_title="Values",
                   plot_bgcolor="white"
               )
...
st.write("Our Seabird SBE 16plus Conductivity-Temperature-Depth (CTD) sensor, often referred to simply as the Seabird 16plus CTD, is a compact, durable oceanographic instrument used for measuring and recording key water column properties in marine environments. It is widely used in scientific research, oceanographic surveys, and environmental monitoring. With the ability to expand the CTD with more sensors, ours has extra attatchments to measure PAR, turbidity, and dissolved oxygen.")
